...
class TestAutomountMapEntry(unittest.TestCase):
    ...
    def testInit(self):
        ...
        self.assertTrue(entry.Verify(), msg="Could not verify seeded AutomountMapEntry")
        self.assertEqual(entry.key, "foo", msg="Entry returned wrong value for name")
        self.assertEqual(
            entry.options, None, msg="Entry returned wrong value for options"
        )
        self.assertEqual(
            entry.location, "/dev/sda1", msg="Entry returned wrong value for location"
        )
